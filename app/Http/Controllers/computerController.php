...
class computerController extends Controller {
    ...
    public function index() {
        ...
        $feedbacks = $dl->getFeedbacksByCategory(2);
        //$reviews =array();
    ...
    public function store(Request $request) {
        $name = $request->input('inputNamePC');
        $price = $request->input('inputPricePC');
        $category = $request->input('inputCategoryPC');
        $capacity = $request->input('inputCapacityPC');
        $model = $request->input('inputModelPC');
        $assurance = $request->input('inputAssurancePC');
        $color = $request->input('inputColorPC');
        $quantity = $request->input('inputQuantityPC');

        $dl = new DataLayer();

        $product = $dl->getProduct($color, $model, $assurance, $capacity, 2);
